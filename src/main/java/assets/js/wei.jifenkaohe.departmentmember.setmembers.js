...
$(function ()
{
    ...
    var a = new Array();
    $.getJSON("/api/jifenkaohe/bumen/members/"+dialogData.id, function(res){
        var departmentMembers=res['entity'];
        //var departmentMembers=entity['departmentMembers'];
        if(departmentMembers.length<1){
            alert("此部门未分配用户组,请到'系统管理模块'给部门分配用户组");
        }
        $.each(departmentMembers, function(n, value) {//获得member数据
        ...
        $.getJSON("/api/jifenkaohe/bumen/showDepartmember/"+dialogData.id,function(members){
...
function save(){
    ...
    $.ajax({
        type: 'POST',
        url: '/api/jifenkaohe/bumen/member/create/'+dialogData.id,
        data: postData,
